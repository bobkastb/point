...
// Функции обработки переменных окружения в тексте
// Функции загрузки конфигурационных файлов в формате JSON с использованием переменных окружения
// Функции для выполнения дочерних процессов
...
export async function Execute(cmd: string, options?: { hide?: number, echo?: number, raise?: number }): Promise<ExecResult> {
    ...
    let code = await new Promise<any>(r => {
        ...
        p.on("exit", code => {
            result.exitcode = code != null ? code : undefined;
            r(code);
            //console.log(`end exec(${cmd})`);
        });
    });
